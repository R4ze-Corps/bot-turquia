    ...
    async run(interaction) {
        try {
            ...
            for (const [msgId, acao] of acoesEmAndamento) {
                if (acao.pingMsgId === "iniciada" || acao.participantes.length > 0) continue;
                acoesEmAndamento.delete(msgId);
            }
            const acoesRemoved = acoesSizeBefore - acoesEmAndamento.size;
            if (acoesRemoved > 0) cleared.push(`Ações abandonadas (${acoesRemoved})`);

            const afterMem = process.memoryUsage().heapUsed;
            const freed = Math.round((dbSizeBefore - afterMem) / 1024 / 1024 * 100) / 100;

            const container = createContainer("#22c55e",
                "**🧹 Cache Limpo**",
                Separator.Default,
                cleared.map(c => `✅ ${c}`).join("\n"),
                Separator.Default,
                freed > 0 ? `💾 **Memória liberada:** ~${freed} MB` : "💾 **Memória:** sem alteração significativa",
            );
