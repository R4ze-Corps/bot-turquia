...
function statusRoleLabel(config: import("#database").StatusRoleConfig, presenceStatus: string, defaultLabel: string): string {
  ...
  async run(interaction) {
    ...
    if (!isConfigured) {
      const container = createContainer("#FFFFFF",
        "**📡 Configurar Cargos de Status**",
        Separator.Default,
        "Configure os cargos para cada status do bot.",
        "Os cargos serão atribuídos automaticamente ao alterar o status.",
        Separator.Default,
