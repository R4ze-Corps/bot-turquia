...
function montarContainerVendasComSelecao(config: { produtos: Produto[]; logChannelId: string | null }, selectedId: string) {
    const produto = config.produtos.find(p => p.id === selectedId)!;
    return montarContainerVendasConfig(config as any, produto);
}
...
createResponder({
    ...
    async run(interaction) {
        const container = createContainer("#FFFFFF",
            "**📢 Selecione o canal de logs de vendas**",
            Separator.Default,
