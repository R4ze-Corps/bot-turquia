...
const sessoesFinalizacaoAcao = new Map<string, string>();
const sessoesEdicaoAcao = new Map<string, string>();
    ...
    async run(interaction) {
        try {
            ...
            for (const pid of acao.participantes) {
                ...
                if (member) member.send(dmMsg).catch(() => {});
            }
    ...
    async run(interaction) {
        try {
            ...
            const modal = new ModalBuilder()
                ...
                .addComponents(new ActionRowBuilder<TextInputBuilder>({ components: [valInput] }));
        ...
        try {
            ...
            const modal = new ModalBuilder()
                ...
                .addComponents(
                    ...
                    new ActionRowBuilder<TextInputBuilder>({ components: [horarioInput] }),
                    new ActionRowBuilder<TextInputBuilder>({ components: [vagasInput] }),
                    new ActionRowBuilder<TextInputBuilder>({ components: [armamentoInput] }),
                    new ActionRowBuilder<TextInputBuilder>({ components: [regrasInput] }),
                );
