    ...
    async run(interaction) {
        try {
            ...
            if (salaAtualizada.entregas.length === 1 && config.cargos) {
                ...
                if (member) {
                    const roleIds = [config.cargos.cargoMetaConcluida, config.cargos.cargoMetaIncompleta, config.cargos.cargoNenhumaEntrega].filter(Boolean) as string[];
                    await member.roles.remove(roleIds).catch(() => {});
                }
            }

            // Build updated log containers
            const logContainer = montarContainerLog(
                config.produtos,
                salaAtualizada.entregas,
                salaAtualizada.memberId,
                salaAtualizada.inicioTimestamp,
                salaAtualizada.ultimaTimestamp,
                salaAtualizada.status,
                salaAtualizada.finalizedBy,
                true,
            );

            const logContainerSimples = montarContainerLog(
                config.produtos,
                salaAtualizada.entregas,
                salaAtualizada.memberId,
                salaAtualizada.inicioTimestamp,
