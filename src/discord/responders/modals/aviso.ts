...
export const avisoStore = new Map<string, { titulo: string; texto: string; channelId: string; guildName: string }>();
...
createResponder({
    ...
    async run(interaction) {
        try {
            ...
            const previewContainer = createContainer("#FFFFFF",
                "📢 **PRÉ-VISUALIZAÇÃO DO AVISO**",
                Separator.Default,
                `**${titulo.toUpperCase()}**\n${texto}`,
                Separator.Default,
                `*Informativos ONE CORE | Servidor: ${interaction.guild?.name ?? "Servidor"}*`,
                createRow(
                    new ButtonBuilder()
