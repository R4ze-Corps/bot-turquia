...
import type { AcaoTemplate } from "#database";

export const selectedAcaoId = new Map<string, string>();
export const regrasSelectedId = new Map<string, string>();

function montarContainerAcoesComSelecao(templates: AcaoTemplate[], selectedId: string) {
    ...
    return createContainer("#FFFFFF",
        ...
        Separator.Default,
        "**Ação selecionada:**",
        `**${template.nome}** — 🔫 ${template.armamento} — 👥 ${template.vagasMin}-${template.vagasMax} vagas`,
        Separator.Default,
        createRow(
            new ButtonBuilder()
