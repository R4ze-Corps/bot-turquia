...
export interface AcaoTemplateOption {
    id: string;
    nome: string;
    armamento: string;
    vagasMin: number;
...
export function montarContainerSelecaoAcao(
    templates: AcaoTemplateOption[],
    vagasOptions?: { min: number; max: number },
    selectedId?: string
) {
    ...
    const vagasOptionsList = vagasOptions
        ? Array.from(
              { length: vagasOptions.max - vagasOptions.min + 1 },
              (_, i) => ({
    ...
    const vagasSelect = new StringSelectMenuBuilder()
        ...
        .setPlaceholder(
            vagasOptions ? "Selecione as vagas..." : "Ação não selecionada"
        )
    ...
    return createContainer(
        ...
        Separator.Default,
        "**Ação**",
        createRow(acaoSelect),
        Separator.Default,
        "**Horário**",
        createRow(horarioSelect),
        Separator.Default,
        "**Vagas**",
        createRow(vagasSelect),
        Separator.Default,
        createRow(avancarBtn)
    );
}
