...
import type { GuildConfig, HierarquiaCargo, AcaoTemplate, PunishmentGuildConfig, UpdateGuildConfig, Produto, VendasGuildConfig, StatusRoleConfig, ReporteGuildConfig, FarmGuildConfig, FarmProduto, AusenciaGuildConfig } from "../../database/index.js";
...
export function montarMenuConfig() {
    ...
    return createContainer("#FFFFFF",
        ...
        Separator.Default,
        createRow(registrarBtn, hierarquiaBtn, acoesBtn),
        createRow(punicoesBtn, farmBtn, updateBtn),
        createRow(vendasBtn, statusBtn, reporteBtn, ausenciaBtn),
        Separator.Default,
        createRow(redefinirBtn),
    );
}

export function montarContainerRegistrar(existing?: GuildConfig | null) {
    return createContainer("#FFFFFF",
        "**Cargo de Registro**\nCargo necessário para usar /registrar",
        ...
        Separator.Default,
        "**Cargo Aprovado**\nCargo atribuído após a aprovação",
        createRow(new RoleSelectMenuBuilder({
        ...
        Separator.Default,
        "**Cargo Aprovado 2**\nSegundo cargo opcional (ou nenhum)",
        createRow(new RoleSelectMenuBuilder({
        ...
        Separator.Default,
        "**Canal de Log**\nCanal onde os registros serão enviados para aprovação",
...
function textoHierarquiaPreview(cargos: HierarquiaCargo[]): string {
    if (cargos.length === 0) return "Nenhum cargo selecionado.";
    const lines = cargos.map(c => `• ${c.nome} (<@&${c.id}>)`);
    return lines.join("\n");
}

export function montarContainerHierarquiaConfig(cargos?: HierarquiaCargo[]) {
    const lista = cargos ?? [];
    const preview = textoHierarquiaPreview(lista);

    const components: (ButtonBuilder | RoleSelectMenuBuilder)[] = [];
    ...
    return createContainer("#FFFFFF",
        ...
        Separator.Default,
        "**Cargos da Hierarquia** (do maior para o menor)",
        Separator.Default,
        preview,
        Separator.Default,
...
export function montarContainerHierarquiaEditado(cargos: HierarquiaCargo[]) {
    return montarContainerHierarquiaConfig(cargos);
}

export function montarContainerAcoesConfig(templates: AcaoTemplate[]) {
...
export function montarContainerUpdateConfig(config: UpdateGuildConfig) {
    const statusLines = [
...
export function montarContainerPunicoesConfig(config: PunishmentGuildConfig) {
    const statusLines = [
        `**📋 Canais**`,
...
export function montarContainerVendasConfig(config: VendasGuildConfig, selectedProduto?: Produto) {
    ...
    if (config.produtos.length > 0) {
        components.push(
            "**Selecione um produto para editar/excluir:**",
            createRow(
                new StringSelectMenuBuilder()
...
export function montarContainerStatusConfig(config: StatusRoleConfig) {
    return createContainer("#FFFFFF",
        "**📡 Configuração de Status**",
        Separator.Default,
        `🟢 Online: ${config.roleOnline ? `<@&${config.roleOnline}>` : "*Não configurado*"}`,
        `🔄 Atualizando: ${config.roleAtualizando ? `<@&${config.roleAtualizando}>` : "*Não configurado*"}`,
        `⚫ Offline: ${config.roleOffline ? `<@&${config.roleOffline}>` : "*Não configurado*"}`,
        `⚠️ Instável: ${config.roleInstavel ? `<@&${config.roleInstavel}>` : "*Não configurado*"}`,
        Separator.Default,
        "**Selecione o cargo para cada status:**",
        Separator.Default,
        createRow(new RoleSelectMenuBuilder()
...
export function montarContainerReporteConfig(config: ReporteGuildConfig) {
    return createContainer("#FFFFFF",
        "**🐛 Configuração de Reporte**",
        Separator.Default,
        `**Categoria de Tickets:** ${config.categoriaId ? `<#${config.categoriaId}>` : "*Não configurada*"}`,
        Separator.Default,
        createRow(new ChannelSelectMenuBuilder()
...
export function montarContainerFarmConfig(config: FarmGuildConfig, selectedProduto?: FarmProduto) {
...
export function montarContainerAusenciaConfig(config: AusenciaGuildConfig) {
    return createContainer("#FFFFFF",
        "**📅 Configuração de Ausência**",
        Separator.Default,
        `📢 Canal de Logs: ${config.logChannelId ? `<#${config.logChannelId}>` : "*Não configurado*"}`,
        `📝 Título do Painel: ${config.panelTitle}`,
        `📄 Descrição: ${config.panelDesc.length > 100 ? config.panelDesc.slice(0, 100) + "..." : config.panelDesc}`,
        Separator.Default,
