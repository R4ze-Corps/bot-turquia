...
createResponder({
    ...
    async run(interaction) {
        try {
            ...
            const ticketContainer = createContainer("#FFFFFF", "**# Atendimento Técnico Iniciado**", Separator.Default, `**Reporte por:** <@${interaction.user.id}>`, Separator.Default, "- Seu reporte de bug foi registrado e já está na fila de análise.\n- Por favor, envie evidências (fotos/vídeos) do erro neste canal.\n- O chamado será encerrado assim que o problema for corrigido\n\n *Aguarde o retorno do desenvolvedor.*", Separator.Default, "**SELECIONE A PRIORIDADE DO PROBLEMA**", createRow(new ButtonBuilder()
                ...
                .setLabel("BAIXA")
                .setEmoji("🟢")
                .setStyle(ButtonStyle.Success)), Separator.Default, createRow(new ButtonBuilder()
...
createResponder({
    ...
    async run(interaction) {
        try {
            const container = createContainer("#FFFFFF", "**🔧 Atendimento em Andamento**", Separator.Default, `**Atendente:** <@${interaction.user.id}>`, Separator.Default, "O desenvolvedor está analisando o problema.\n\nAcompanhe este canal para atualizações.", Separator.Default, createRow(new ButtonBuilder()
                .setCustomId("reporte/fechar")
                .setLabel("Fechar")
                .setEmoji("🔒")
                .setStyle(ButtonStyle.Secondary)));
...
createResponder({
    ...
    async run(interaction) {
        try {
            ...
            const container = createContainer("#FFFFFF", "**🔒 Confirmar Fechamento**", Separator.Default, "Tem certeza que deseja fechar este chamado?", Separator.Default, createRow(new ButtonBuilder()
...
createResponder({
    ...
    async run(interaction) {
        try {
            ...
            const container = createContainer("#FFFFFF", "**🔒 Atendimento Encerrado**", Separator.Default, `**Encerrado por:** <@${interaction.user.id}>`, Separator.Default, "Este chamado foi encerrado.\n\nO canal será fechado em instantes...");
            await interaction.update({ components: [container] });
            setTimeout(async () => {
                try {
...
createResponder({
    ...
    async run(interaction) {
        try {
            ...
            const reporterId = channel && "topic" in channel ? channel.topic ?? interaction.user.id : interaction.user.id;
            const container = createContainer("#FFFFFF", "**# Atendimento Técnico Iniciado**", Separator.Default, `**Reporte por:** <@${reporterId}>`, Separator.Default, "- Seu reporte de bug foi registrado e já está na fila de análise.\n- Por favor, envie evidências (fotos/vídeos) do erro neste canal.\n- O chamado será encerrado assim que o problema for corrigido\n\n *Aguarde o retorno do desenvolvedor.*", Separator.Default, "**SELECIONE A PRIORIDADE DO PROBLEMA**", createRow(new ButtonBuilder()
                .setCustomId("reporte/prioridade/alta")
                .setLabel("ALTA")
                .setEmoji("🔴")
                .setStyle(ButtonStyle.Danger), new ButtonBuilder()
    ...
    createResponder({
        ...
        async run(interaction) {
            try {
                ...
                if (!ch?.isTextBased())
                    return;
                const reporterId = ch && "topic" in ch ? ch.topic ?? interaction.user.id : interaction.user.id;
                await ch.setName(`${data.emoji}・${data.label.toLocaleLowerCase()}`);
                await interaction.update({
                    components: [
                        createContainer("#FFFFFF", "**# Atendimento Técnico Iniciado**", Separator.Default, `**Reporte por:** <@${reporterId}>`, Separator.Default, "- Seu reporte de bug foi registrado e já está na fila de análise.\n- Por favor, envie evidências (fotos/vídeos) do erro neste canal.\n- O chamado será encerrado assim que o problema for corrigido\n\n *Aguarde o retorno do desenvolvedor.*", Separator.Default, `**PRIORIDADE DEFINIDA:** ${data.emoji} ${data.label}`, Separator.Default, createRow(new ButtonBuilder()
                            .setCustomId("reporte/iniciar-atendimento")
                            .setLabel("Iniciar Atendimento")
                            .setEmoji("🔧")
                            .setStyle(ButtonStyle.Secondary), new ButtonBuilder()
