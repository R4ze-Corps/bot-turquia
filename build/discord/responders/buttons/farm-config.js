...
function montarContainerFarmComSelecao(config, selectedId) {
    const produto = config.produtos.find(p => p.id === selectedId);
    return montarContainerFarmConfig(config, produto);
}
createResponder({
...
createResponder({
    ...
    cache: "cached",
    async run(interaction) {
        const container = createContainer("#FFFFFF", "**📢 Selecione o canal de logs de farm**", Separator.Default, createRow(new ChannelSelectMenuBuilder({
...
createResponder({
    ...
    cache: "cached",
    async run(interaction) {
        const container = createContainer("#FFFFFF", "**📁 Selecione a categoria para as salas de farm**", Separator.Default, createRow(new ChannelSelectMenuBuilder({
...
createResponder({
    ...
    async run(interaction) {
        ...
        const container = createContainer("#FFFFFF", "**🎭 Configurar Cargos de Farm**", Separator.Default, "**✅ Meta Concluída**", createRow(new RoleSelectMenuBuilder({
            customId: "config/farm/cargo/meta-concluida",
            placeholder: cargos.cargoMetaConcluida ? "<cargo selecionado>" : "Selecione o cargo...",
            maxValues: 1,
        })), Separator.Default, "**❌ Meta Incompleta**", createRow(new RoleSelectMenuBuilder({
            customId: "config/farm/cargo/meta-incompleta",
            placeholder: cargos.cargoMetaIncompleta ? "<cargo selecionado>" : "Selecione o cargo...",
            maxValues: 1,
        })), Separator.Default, "**🚫 Nenhuma Entrega**", createRow(new RoleSelectMenuBuilder({
