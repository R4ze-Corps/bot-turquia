...
const editingAcaoId = new Map();
function parseVagas(input) {
    const parts = input.split("-").map(s => parseInt(s.trim()));
    if (parts.length === 1 && !isNaN(parts[0]) && parts[0] >= 2)
        return { min: parts[0], max: parts[0] };
    if (parts.length === 2 && !isNaN(parts[0]) && !isNaN(parts[1]) && parts[0] >= 2 && parts[1] >= parts[0])
...
createResponder({
    ...
    async run(interaction) {
        try {
            ...
            function extrairMinMax(bandidos) {
            ...
            for (const item of items) {
                ...
                if (vagasMin !== null && vagasMax !== null) {
                    if (!nome || !armamento || vagasMin < 2 || vagasMax < vagasMin)
                        continue;
...
createResponder({
    ...
    async run(interaction) {
        ...
        const container = createContainer("#FFFFFF", "**📜 Selecione a ação para configurar regras**", Separator.Default, createRow(select), Separator.Default, createRow(backBtn));
        await interaction.update({ components: [container] });
    }
});
createResponder({
