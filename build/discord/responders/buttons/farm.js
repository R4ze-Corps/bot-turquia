...
function extrairDadosNickname(memberName) {
...
createResponder({
    ...
    async run(interaction) {
        try {
            ...
            await setFarmSala(salaKey, sala);
            // Build containers
            const salaContainer = montarContainerLog(config.produtos, [], sala.memberId, sala.inicioTimestamp, sala.ultimaTimestamp, "meta_encerrada", interaction.user.id, true);
            const logContainer = montarContainerFarmFinalizado(config.produtos, sala.entregas, sala.memberId, sala.inicioTimestamp, sala.ultimaTimestamp, interaction.user.id, false);
            ...
            if (member && config.cargos) {
                ...
                let cargoId = null;
                if (todasBatidas)
                    cargoId = config.cargos.cargoMetaConcluida;
                else if (algumaEntrega)
                    cargoId = config.cargos.cargoMetaIncompleta;
                else
                    cargoId = config.cargos.cargoNenhumaEntrega;
                if (cargoId) {
...
createResponder({
    ...
    async run(interaction) {
        ...
        if (sala) {
            const config = await getFarmConfig(interaction.guild.id);
            const container = montarContainerLog(config.produtos, sala.entregas, sala.memberId, sala.inicioTimestamp, sala.ultimaTimestamp, sala.status, sala.finalizedBy);
            await interaction.update({ components: [container] });
        }
        else {
