...
createResponder({
    ...
    async run(interaction) {
        try {
            ...
            const container = createContainer("#FFFFFF", sessao.tipo === "PROMOÇÃO" ? "**📈 Promover**" : "**📉 Rebaixar**", Separator.Default, `**Membro:** <@${memberId}>`, Separator.Default, "**Novo Cargo**", createRow(select));
...
createResponder({
    ...
    async run(interaction) {
        try {
            ...
            for (const c of config.cargosPromocao)
                allIds.add(c.id);
            for (const c of config.cargosRebaixamento)
                allIds.add(c.id);
            if (config.geraisRoleId)
                allIds.add(config.geraisRoleId);
