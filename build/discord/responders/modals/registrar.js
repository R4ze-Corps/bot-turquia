    ...
    async run(interaction) {
        try {
            ...
            if (logChannel?.isTextBased()) {
                const logContainer = createContainer("#FFFFFF", "**📋 Novo Registro Pendente**", Separator.Default, `**Candidato:** <@${interaction.user.id}>`, `**Nome:** ${nome}`, `**ID:** ${id}`, `**Telefone:** ${telefone}`, `**Recrutador:** <@${pending.recruiterId}>`, `**Data:** <t:${Math.floor(Date.now() / 1000)}:F>`, Separator.Default, createRow(new ButtonBuilder({
            ...
            const confirmContainer = createContainer(constants.colors.success, "**📋 Registro enviado para análise!**", Separator.Default, `**Nome:** ${nome}`, `**ID:** ${id}`, `**Telefone:** ${telefone}`, `**Recrutador:** <@${pending.recruiterId}>`, Separator.Default, "Aguarde a aprovação de um responsável.");
            await interaction.reply({
