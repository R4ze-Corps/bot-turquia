...
export const acoesEmAndamento = new Map();
export async function persistAcao(msgId) {
    const acao = acoesEmAndamento.get(msgId);
    if (acao) {
        await setActiveAcao(msgId, acao);
    }
}
export async function deletePersistedAcao(msgId) {
    acoesEmAndamento.delete(msgId);
    await deleteActiveAcao(msgId);
}
export function montarContainerAcao(acao, options) {
    ...
    if (!isFinal) {
        if (!isIniciada) {
            components.push(createRow(new ButtonBuilder()
                ...
                .setLabel("Encerrar")
                .setEmoji("🚫")
                .setStyle(ButtonStyle.Secondary)), Separator.Default, createRow(new ButtonBuilder()
